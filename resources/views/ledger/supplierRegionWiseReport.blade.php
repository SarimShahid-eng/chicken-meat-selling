@extends('partials.app', ['title' => 'Region-wise Supplier Ledger'])
...
@section('content')
    <div class="max-w-6xl mx-auto space-y-6 animate-fade-in">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Region-wise Supplier Ledger</h1>
                <p class="text-sm text-gray-500 mt-1">
                    Track consolidated purchase histories, debits, credits, and balances across regional suppliers.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6">
            <form method="POST" action="{{ route('ledger.regionSupplierReport') }}"
        ...
        @if (request('region_id'))
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                            ...
                            <tr class="bg-amber-50/40 font-medium text-amber-900">
                                ...
                                <td class="px-6 py-3.5 font-bold">All Regional Suppliers</td>
                            ...
                            @forelse($ledgerEntries as $entry)
                                ...
                                <tr class="hover:bg-gray-50/70 transition-colors">
                                    ...
                                    <td class="px-6 py-3.5 font-semibold text-gray-800">{{ $entry->supplier_name }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center text-gray-400">
                                        ...
                                        No ledger activity transactions logged for this region within the selected date
                                        range.
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-100 p-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-center">
                    ...
                    <!-- Total Credit Sum -->
                    <div class="bg-gray-50/80 rounded-lg p-3 border border-gray-100 text-right">
                        <span class="text-xs font-semibold text-gray-500 uppercase tracking-wide block">Net Change</span>
                        <span class="text-base font-bold text-green-600">Rs. {{ number_format($creditSum - $debitSum, 2) }}</span>
                    </div>

                    <!-- Closing Balance -->
                    <div class="bg-slate-900 rounded-lg p-3 text-right">
                        <span class="text-xs font-semibold text-amber-400 uppercase tracking-wide block">Closing Balance</span>
