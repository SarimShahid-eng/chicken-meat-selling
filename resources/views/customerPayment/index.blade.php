...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">CustomersPayment</h1>
                <p class="text-gray-500 mt-1 text-sm">Manage your chicken meat customers payment and stock levels</p>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse text-left text-sm text-gray-500">
                    ...
                    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                        @forelse ($customersPayments as $customerPayment)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-gray-500">
                                    <div class="max-w-xs truncate" title="{{ $customerPayment->amount }}">
                                        {{ $customerPayment->amount }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-gray-500">
                                    <div class="max-w-xs truncate" title="{{ $customerPayment->description }}">
                                        {{ $customerPayment->description ?? 'No description available.' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 font-medium text-gray-900">
                                    {{ $customerPayment->date->format('Y-m-d') }}
                                </td>
                                <td class="px-6 py-4 font-medium text-gray-900">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="inline-flex items-center gap-3">
                                        <a href="{{ route('customersPayment.edit', ['payment' => $customerPayment->id]) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 font-medium">No customersPayments found</p>
